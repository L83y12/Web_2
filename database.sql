...
ADD COLUMN `reset_expired` DATETIME DEFAULT NULL;

-- order
ALTER TABLE `orders`
MODIFY `status` ENUM('pending','confirmed','delivered','cancelled') DEFAULT 'pending';

ALTER TABLE `orders`
ADD COLUMN `ward` VARCHAR(255) AFTER `address`;
